...
appointments[j] = "Kate";
j++;
appointments[j] = "Thomas";
j++;
appointments[j] = "Mike";
j++;
appointments[j] = "Jose";
j++;
appointments[j] = "Julia";
j++;
appointments[j] = "Martin";
j++;
appointments[j] = "Dafne";
j++;
appointments[j] = "Monica";
j++;
appointments[j] = "Jessica";
j++;
appointments[j] = "Carla";
j++;
appointments[j] = "Peter";
j++;
appointments[j] = "Rachel";
j++;
appointments[j] = "Homer";
j++;
appointments[j] = "Margie";
j++;
appointments[j] = "Lisa";
j++;
appointments[j] = "Claire";
j++;
appointments[j] = "Eddie";
j++;
appointments[j] = "Max";
j++;
appointments[j] = "George";
j++;
appointments[j] = "Meggan";
j++;
appointments[j] = "Bill";
j++;
appointments[j] = "Simon";
j++;
appointments[j] = "Oswald";
j++;
appointments[j] = "Walter";
j++;
appointments[j] = "Felix";
j++;
appointments[j] = "Carlton";
j++;
appointments[j] = "Roy";
j++;
appointments[j] = "Grace";
j++;
appointments[j] = "Wynona";
j++;
appointments[j] = "Burns";
j++;
appointments[j] = "Flanders";
j++;
appointments[j] = "Edna";
j++;
appointments[j] = "Milhouse";
j++;
appointments[j] = "Nelson";
j++;
appointments[j] = "Barney";
j++;
appointments[j] = "Carl";
j++;
appointments[j] = "Brian";
j++;
appointments[j] = "Selma";
j++;
appointments[j] = "Veronica";
j++;
appointments[j] = "Rodney";
j++;
appointments[j] = "Mona";
j++;
appointments[j] = "Norbert";
j++;
appointments[j] = "Philip";
j++;
appointments[j] = "Agnes";
j++;
appointments[j] = "Larry";
j++;
appointments[j] = "Waylon";
j++;
appointments[j] = "Elen";
j++;
...
function updateGraph(param) {
  var widthBar =
    (parseInt($("counter ." + param + " value").html()) * 100) / clientsLimit;
  //console.log(widthBar+"%");
  $("performance ." + param).html(
    parseInt($("counter ." + param + " value").html())
  );
...
$(document).ready(function () {
  ...
  $("line").on("click", "obj O_Act button.btnExclude", function () {
    var confirmQuestion = confirm(
      "Do you want to exclude " +
        $(this).parent().parent().find("O_Name").html() +
        " from the line?"
    );
    ...
    if (confirmQuestion == true) {
      ...
      for (i = 0; i < $lineItens.length; i++) {
        if (ObjPos < parseInt($lineItens.eq(i).parent().find("O_pos").html())) {
          ...
          var NovaPos =
            parseInt($lineItens.eq(i).parent().find("O_pos").html()) - 1;
          $lineItens
            .eq(i)
            .parent()
            .find("O_pos")
      ...
      $("counter .schedule value").html(
        parseInt($("counter .schedule value").html()) - 1
      );
  ...
  $("line").on("click", "obj O_Act button.btnStart", function () {
    if ($("clientPanel obj").length == MaxStartedAppointments) {
      alert(
        "You have reached the maximum number of started appointments. Please, wait until we have a free position or finish one of the started ones."
      );
      return false;
    }
  ...
  $("clientPanel").on("click", "obj O_Act button.btnFinish", function () {
    var confirmQuestion = confirm(
      "Do you want to finish " +
        $(this).parent().parent().find("clientName").html() +
        "'s appointment?"
    );
  ...
  $("actionLine .add").click(function () {
    ...
    if (newClient === "" || newClient === null) {
      ...
      if (index % clientsLimit == 1) {
        ...
        data.setDate(
          data.getDate() +
            parseFloat($("line obj").not(".break").length / clientsLimit)
        );
        console.log(data.toLocaleDateString());

        html =
          '<obj class="break">' + data.toLocaleDateString() + "</obj>" + html;

        //lineNavigator
        $("lineNavigator").append(
          "<day>" +
            data.toLocaleDateString().slice(0, 2) +
            "<sup>" +
            data.toLocaleDateString().slice(2, 5) +
            "</sup>" +
            "</day>"
        );
      }
  ...
  $("lineNavigator").on("click", "day", function () {
    ...
    $("html, body").animate(
      {
        scrollTop:
          $("line obj.break").eq(dayIndex).offset().top - headerHeight - 20,
      },
      1000
    );
  });
